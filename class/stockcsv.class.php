...
class Tstockcsv extends TObjetStd
{
	/**
	 * Draft status
	 */
	...
	public function __construct()
	{
		...
		$this->set_table(MAIN_DB_PREFIX.'stockcsv');
...
//		$this->setChild('TstockcsvChild','fk_stockcsv');
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$mask = !empty($conf->global->stockcsv_REF_MASK) ? $conf->global->stockcsv_REF_MASK : 'MM{yy}{mm}-{0000}';
		$numero = get_next_value($db, $mask, 'stockcsv', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showstockcsv") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;
        
        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/stockcsv/card.php', 1).'?id='.$this->getId(). $get_params .$linkclose;
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='stockcsvStatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='stockcsvStatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='stockcsvStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='stockcsvStatusAccepted'; $shortkeytrans='Accepted'; }
	...
	function ListArrayWareHouse(){

		global $db;
		$wharehouse = new FormProduct($db);
		$arr = array();

		// request the Id / Label from warehouse
	...
	function ListArrayCategorie()
	{

		global $db;
		$arr = array();

		// request the Id / Label from categorie
 ...
 */
class stockcsv extends Tstockcsv
{
	private $PDOdb;
...
/*
class TstockcsvChild extends TObjetStd
{
	public function __construct()
	{
		$this->set_table(MAIN_DB_PREFIX.'stockcsv_child');
		
		$this->add_champs('fk_stockcsv', array('type' => 'integer', 'index' => true));
